        ...
        <section className="work-section mb-12 md:mb-32">
            ...
            <div className="flex justify-center ">
              <div className=" flex gap-x-4 md:gap-x-8 text-white md:text-2xl mb-16">
                  <button className={`${category === '' ? "bg-[#66FCF1] py-2 px-8" : '' }`} onClick={ () => setCategory('')}>All</button>
                  <button className={`${category === 'Photography' ? "bg-[#66FCF1] py-2 px-8" : '' }`} onClick={ () => setCategory('Photography')}>Photography</button>
                  <button className={`${category === 'Architecture' ? "bg-[#66FCF1] py-2 px-8" : '' }`} onClick={ () => setCategory('Architecture')}>Architecture</button>
                  <button className={`${category === 'Design' ? "bg-[#66FCF1] py-2 px-8" : '' }`} onClick={ () => setCategory('Design')}>Design</button>
              </div>
            </div>
            <div className="grid grid-cols-12 gap-4 px-12 md:px-40">
                {
                    selectedWorks.map((data) => {
                        return <div className="col-span-12 md:col-span-6 relative group" key={data.id}>
                            ...
                            <div className="absolute flex items-center justify-center inset-0 bg-black/0 group-hover:bg-black/50">
                              <p className="text-white text-bold text-2xl opacity-0 group-hover:opacity-100">{data.title}</p>
                            </div>
                        </div>
                    })
                }
